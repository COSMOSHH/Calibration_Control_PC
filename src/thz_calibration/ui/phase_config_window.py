...
class PhaseConfigWindow(QMainWindow):
    ...
    def _build_phase_queue(self, phases: dict[int, float]) -> list[FeedState]:
        ...
        所有相位统一按 [0, 360) deg 取模；没有独立使能控件时四路默认 enabled。
        """
        return [
            FeedState(
                feed_id=feed_id,
    ...
    def _build_send_queue_with_feed1_zero(self) -> tuple[list[FeedState], float]:
        ...
        feed1_phase = self._normalize_feed_phase(feed1.phase_deg)
        ...
        return [
            FeedState(
                ...
                phase_deg=self._normalize_feed_phase(state.phase_deg + common_offset),
